...
<div class="space-y-6">
    ...
    @php
        $thiBandColors = [
            'green'  => ['border' => 'border-[#4CAF50]', 'badge' => 'bg-green-100 text-green-700', 'dot' => 'bg-[#4CAF50]'],
            'yellow' => ['border' => 'border-yellow-400', 'badge' => 'bg-yellow-100 text-yellow-700', 'dot' => 'bg-yellow-400'],
            'orange' => ['border' => 'border-orange-400', 'badge' => 'bg-orange-100 text-orange-700', 'dot' => 'bg-orange-400'],
            'red'    => ['border' => 'border-red-500',    'badge' => 'bg-red-100 text-red-700',    'dot' => 'bg-red-500'],
        ];
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6" data-thi-chart="{{ $weatherTrend->values()->toJson() }}">
        <div class="bg-white rounded-lg shadow-md p-6 border-l-4 {{ $bandColors['border'] }}">
            <div class="flex items-center justify-between mb-2">
                <span class="text-gray-600">Current Weather</span>
            ...
            @if($latestWeather && $latestWeather->temp_mean !== null)
                <div class="text-3xl font-bold text-gray-800">{{ number_format($latestWeather->temp_mean, 1) }}&deg;C</div>
                ...
                <p class="text-xs text-gray-400 mt-2">Average temperature &mdash; SPC Farm Magalang</p>
                <div class="mt-3 pt-3 border-t border-gray-100 flex flex-wrap gap-x-3 gap-y-1">
                    @foreach(config('weather.thi_bands') as $band)
                        <span class="flex items-center gap-1 text-xs text-gray-500">
                            <span class="w-2 h-2 rounded-full {{ $thiBandColors[$band['color']]['dot'] }}"></span>
                            {{ $band['label'] }}
                        </span>
                    @endforeach
                </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6 lg:col-span-2">
            <div class="mb-4">
                <h2 class="text-lg font-bold text-gray-800">Temperature Trend</h2>
...
(function () {
    ...
    // Temperature Trend — small line chart. Solid = observed (source='archive'),
    // dashed = Open-Meteo's own short-range forecast (source='forecast') —
    // context only, never a prediction generated by this app. Plots temp_mean
    // (°C), not the raw THI index (3S — THI reads as meaningless to staff).
    const thiContainer = document.querySelector('[data-thi-chart]');
    const thiData       = JSON.parse(thiContainer?.getAttribute('data-thi-chart') || '[]').filter(d => d.temp_mean !== null);
    const thiCtx        = document.getElementById('thiChart');
    if (thiCtx && thiData.length) {
        const thiObserved = thiData.map(d => d.is_forecast ? null : d.temp_mean);
        const thiForecast = thiData.map(d => d.is_forecast ? d.temp_mean : null);
        ...
        new Chart(thiCtx, {
            ...
            options: {
                responsive: true,
                plugins: { legend: { display: thiForecast.some(v => v !== null), labels: { font: { size: 10 } } } },
                scales: {
                    x: { ticks: { maxTicksLimit: 10, font: { size: 11 } }, grid: { display: false } },
                    y: { ticks: { font: { size: 11 } }, beginAtZero: false }
                }
