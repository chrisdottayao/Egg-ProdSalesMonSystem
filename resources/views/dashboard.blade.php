...
(function () {
    ...
    // THI Trend — small line chart. Solid = observed (source='archive'),
    // dashed = Open-Meteo's own short-range forecast (source='forecast') —
    // context only, never a prediction generated by this app.
    ...
    if (thiCtx && thiData.length) {
        const thiObserved = thiData.map(d => d.is_forecast ? null : d.thi);
        const thiForecast = thiData.map(d => d.is_forecast ? d.thi : null);
