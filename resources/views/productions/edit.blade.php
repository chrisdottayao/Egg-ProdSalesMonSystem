            ...
            <div class="bg-white shadow-sm rounded-lg p-6">
                <form method="POST" action="{{ route('productions.update', $production) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="flock_id" name="flock_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                            ...
                            @foreach($flocks as $flock)
                                <option value="{{ $flock->id }}" {{ old('flock_id', $production->flock_id) == $flock->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <x-text-input id="date" name="date" type="date" class="mt-1 block w-full" :value="old('date', $production->date->format('Y-m-d'))" required />
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <x-text-input id="total_eggs" name="total_eggs" type="number" min="0" class="mt-1 block w-full" :value="old('total_eggs', $production->total_eggs)" required />
                            <x-input-error :messages="$errors->get('total_eggs')" class="mt-1" />
                        </div>
                        <div>
                            <x-input-label for="cracked_eggs" value="Cracked Eggs *" />
                            <x-text-input id="cracked_eggs" name="cracked_eggs" type="number" min="0" class="mt-1 block w-full" :value="old('cracked_eggs', $production->cracked_eggs)" required />
                    ...
                    <div class="mb-6">
                        ...
                        <textarea id="notes" name="notes" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('notes', $production->notes) }}</textarea>
